...
add_new_dataset <- function(root_url, ddh_credentials = list(cookie = dkanr::get_cookie(), token = dkanr::get_token())) {
  ...
  for(i in 1:length(temp[["results"]])) {
    ...
    resp_res <- ddhconect::create_resource(credentials = ddh_credentials, body = json_res, root_url = root_url)
